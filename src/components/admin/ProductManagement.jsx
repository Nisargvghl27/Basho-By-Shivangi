...
import { 
  Plus, Search, Edit, Trash2, Package,
  X, Loader2, Sparkles, Tag
} from "lucide-react";
import { fetchAllProducts, createProduct, updateProduct, deleteProduct } from "../../lib/productService";
import { generateProductDescription, generateImageTags } from "../../lib/gemini";
...
export default function ProductManagement() {
  ...
  const handleGenerateTags = async () => {
    ...
    try {
      const tags = await generateImageTags(formData.imageUrl);
      if (tags) {
        const currentTags = formData.tags ? formData.tags + ", " : "";
        setFormData(prev => ({ ...prev, tags: currentTags + tags }));
      } else {
        alert("Could not analyze image. Note: URL must be publicly accessible.");
      }
    } catch (error) {
      console.error(error);
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                 ...
                 <textarea 
                    ...
                    onChange={handleInputChange} 
                    className="w-full p-2 border rounded-lg bg-transparent h-24" 
                    placeholder="Enter details manually or click Auto-Write..."
                 />
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                    ...
                    <button
                        ...
                        disabled={isGeneratingTags || !formData.imageUrl}
                        className="text-xs flex items-center gap-1 text-purple-600 hover:text-purple-700 disabled:opacity-50"
                    >
                        {isGeneratingTags ? <Loader2 className="w-3 h-3 animate-spin" /> : <Tag className="w-3 h-3" />}
                        {isGeneratingTags ? "Analyzing..." : "Auto-Tag from Image"}
                    </button>
                </div>
                <input 
                    name="tags" 
                    ...
                    onChange={handleInputChange} 
                    className="w-full p-2 border rounded-lg bg-transparent" 
                    placeholder="mugs, handmade, gift" 
                />
              </div>
