              ...
              <div className={`flex gap-6 whitespace-nowrap ${isMarqueeHovered ? 'pause-animation' : 'animate-marquee'}`}>
                {[...processItems, ...processItems, ...processItems].map((item, idx) => (
                  ...
                  >
                    ...
                    {/* Play Icon (Shows only for videos) */}
                    {item.type === 'video' && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className="size-16 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white border border-white/20 scale-90 group-hover/video:scale-110 group-hover/video:bg-clay/20 transition-all duration-500 shadow-2xl">
                                <Play size={32} fill="currentColor" />
                            </div>
                        </div>
                    )}
