...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    // 4. Listen to Auth State and prefill
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (userSnap.exists()) {
            ...
            setShippingInfo(prev => {
              const updated = { ...prev };
              // We prioritize currentUser.email over localStorage since they are logged in
              if (currentUser.email) updated.email = currentUser.email;

              if (!prev.firstName && firstName) updated.firstName = firstName;
              if (!prev.lastName && lastName) updated.lastName = lastName;
              if (!prev.phone && (userData.phoneNumber || userData.phone)) {
                updated.phone = userData.phoneNumber || userData.phone;
              }
              if (userData.defaultAddress) {
                const addr = userData.defaultAddress;
                if (!prev.address && addr.street) updated.address = addr.street;
                if (!prev.apartment && addr.apartment) updated.apartment = addr.apartment;
                if (!prev.city && addr.city) updated.city = addr.city;
                if (!prev.state && addr.state) updated.state = addr.state;
                if (!prev.zipCode && addr.pincode) updated.zipCode = addr.pincode;
                if (!prev.country && addr.country) updated.country = addr.country;
              }
              return updated;
            });
          }
        } catch (error) {
          console.error("Error loading user profile details at checkout:", error);
        }
      }
    });
  ...
  // Scroll to top when changing steps (e.g. from Shipping to Payment)
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [step]);
  ...
  const handleOrderSuccess = async () => {
    // Decrement stock in Firestore for all checked out items
    try {
      for (const item of filteredCartItems) {
        if (item.id) {
          ...
          console.log(`Updated stock on client for product ${item.id}: decreased by ${item.quantity}`);
        }
      }
    } catch (stockError) {
      console.error('Failed to update stock on client:', stockError);
    }
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      if (order.id) {
        ...
        console.log('[Checkout] Razorpay key loaded:', razorpayKey ? razorpayKey.slice(0, 12) + '...' : 'MISSING');
        if (!razorpayKey) {
        ...
        rzp.on('payment.failed', function (response) {
          // Log full response to diagnose empty error objects
          console.error('[Razorpay] payment.failed full response:', JSON.stringify(response));
          setLoading(false);
          const description = response?.error?.description || response?.error?.reason || 'Payment was declined or cancelled.';
          ...
          <div className="flex items-center justify-center mb-10">
            <div className="flex items-center space-x-4">
              ...
              <div className={`flex items-center ${step >= 1 ? 'text-clay' : 'text-stone-500'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 1 ? 'border-clay bg-clay text-white' : 'border-stone-600 bg-transparent'
                  }`}>1</div>
              ...
              <div className={`flex items-center ${step >= 2 ? 'text-clay' : 'text-stone-500'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 2 ? 'border-clay bg-clay text-white' : 'border-stone-600 bg-transparent'
                  }`}>2</div>
                ...
                <form onSubmit={handleShippingSubmit} className="bg-charcoal-light rounded-xl p-6 md:p-8 border border-border-subtle shadow-lg">
                  ...
                  <div className="mb-8">
                    ...
                    <div className="space-y-3">
                      {/* Standard */}
                      <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${shippingMethod === 'standard' ? 'border-clay bg-clay/10' : 'border-border-subtle hover:border-stone-500'
                        }`}>
                        <input
                          type="radio"
                          name="shipping"
                      ...
                      {/* Express */}
                      <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${shippingMethod === 'express' ? 'border-clay bg-clay/10' : 'border-border-subtle hover:border-stone-500'
                        }`}>
                        <input
                          type="radio"
                          name="shipping"
                ...
                <form onSubmit={handlePaymentSubmit} className="bg-charcoal-light rounded-xl p-6 md:p-8 border border-border-subtle shadow-lg animate-fade-in-up">
                  ...
                  <div className="mb-8">
                    ...
                    <div className="space-y-4">
                      {/* Razorpay Option */}
                      <label className={`relative flex items-start p-5 border rounded-xl cursor-pointer transition-all ${paymentMethod === 'razorpay' ? 'border-clay bg-clay/10' : 'border-border-subtle hover:border-stone-500'
                        }`}>
                      ...
                      {/* COD Option */}
                      <label className={`relative flex items-start p-5 border rounded-xl cursor-pointer transition-all ${paymentMethod === 'cod' ? 'border-clay bg-clay/10' : 'border-border-subtle hover:border-stone-500'
                        }`}>
                        <div className="flex items-center h-5">
                          <input
                            type="radio"
